...
int main(int argc, char * argv[])
{
	...
	int n,niter,nvariables,nbeta,nbetagroup,ntheta,nsavecoda,npsi,RE,movetype,varselect,fixed,maxinteraction,sampinitheta;
	...
	j=fscanf(PriorFile, "%d", &sampinitheta);j=fscanf(PriorFile, "%s", dummy);
	fclose(PriorFile);
...
/*		}*/
		...
		if(sampinitheta==1)
		{
			...
			//initialise cut-points from data
/*			for(j=0;j<ntheta;j++) theta[j]=0.0;*/
/*			for(i=0;i<n;i++) theta[(int) variables[index2(i,nbetagroup+1,n)]]+=1.0;*/
/*			for(j=1;j<ntheta;j++) theta[j]=theta[j]+theta[j-1];*/
/*			for(j=0;j<ntheta;j++) theta[j]=theta[j]/((double) n);*/
/*			for(j=0;j<ntheta;j++) theta[j]=exp(theta[j])/(1.0+exp(theta[j]));*/
		}
		else
		{
			//read in initial values for cut-points
			DataFile = fopen("theta.txt","r");
			if(!DataFile)
			{
				printf("Could not open theta file\n");
				return 1;
			}
			for(i=0;i<ntheta;i++) k=fscanf(DataFile,"%lf",&theta[i]);
			fclose(DataFile);
		}
		//check validity
		valid=1;
