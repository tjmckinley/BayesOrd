...
plot.fitted.bayesord <- function(x, variable = "all", ...) {
    if (class(x) != "fitted.bayesord" & class(x) != "predict.bayesord") 
        stop("'x' is not a 'fitted.bayesord' or a 'predict.bayesord' object")
    if (!is.character(variable)) 
        stop("'variable' not a character")
    
    if (variable == "all") {
        # extract variables to plot from x
        variable <- x$var.info$variables[-1]
    } else {
        object1 <- match(variable, x$var.info$variables[-1])
        if (length(object1[is.na(object1)]) > 1) 
            stop(paste("One or more of 'variable' not found in data set", sep = ""))
    }
    object1 <- numeric(length(variable))
    for (i in 1:length(variable)) object1[i] <- ifelse(!is.factor(x$var.info$orig.data[, 
        match(variable[i], colnames(x$var.info$orig.data))]), 0, 1)
    nonfactor <- variable[object1 == 0]
    if (length(nonfactor) > 0) 
        cat(paste("The following variables are non-categorical and have been removed:\n", 
            paste(nonfactor, collapse = "\n"), "\n"))
    variable <- variable[object1 == 1]
    ...
    for (i in 1:length(variable)) {
        ...
        for (j in 1:length(levels(var[, 1]))) {
            temp <- x$fits[var[, 1] == levels(var[, 1])[j]]
